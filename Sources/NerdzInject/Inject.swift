...
///
/// The wrapper captures the container that was ``NerdzInject/current`` when it
/// was initialized (when the enclosing object was constructed) and resolves from
/// that container on every read. The value is `nil` when nothing is registered
/// for the key. Set `allowRegister` to `true` to register the assigned value on
/// write.
@propertyWrapper public struct Inject<T> {

    private let identifier: String
    private let allowRegister: Bool
    private let container: NerdzInject
    ...
    ///
    /// Reading resolves from the container captured at initialization. Writing
    /// registers the new value into that container when `allowRegister` is
    /// `true`, and does nothing otherwise.
    public var wrappedValue: T? {
        get {
            container.resolve(by: identifier)
        }
        ...
        nonmutating set {
            ...
            container.registerObject(newValue, for: identifier)
        }
    }
}
...
///
/// The wrapper captures the container that was ``NerdzInject/current`` when it
/// was initialized (when the enclosing object was constructed) and force
/// resolves from that container on every read. Set `allowRegister` to `true` to
/// register the assigned value on write.
///
...
@propertyWrapper public struct ForceInject<T> {
    ...
    ///
    /// Reading force resolves from the container captured at initialization.
    /// Writing registers the new value into that container when `allowRegister`
    /// is `true`, and does nothing otherwise.
    ///
    /// > Warning: Reading crashes when nothing is registered for the key.
    public var wrappedValue: T {
        get {
            container.forceResolve(by: identifier)
        }
